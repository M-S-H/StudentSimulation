...
possible = [:centrality, :delay, :blocking, :reachability, :credits]
...
for y in Y
    ...
    for i=1:length(possible)
        for c in combinations(possible, i)
            ...
            # data[:new] = zeros(size(data)[1])
            # for feature in c
            #     data[:new] += data[feature]
            # end
            # fm = Formula(y, :new)
    ...
    writetable("./results/$(curricula)/$(y)_$(difficulty)_combinations_added.csv", results)
end